        ...
        <div class="row">
            ...
            <div class="card card-primary">
                <div class="card-header" style="background-color:#343a40;">
                    <h3 class="card-title" style="font-size:16px;"><i class="nav-icon fas fa-user-alt"></i>&nbsp;&nbsp;Enrollment Module</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                                    ...
                                    echo'<tr>';
                                        ...
                                        echo'<td>';
                                            ...
                                            if($y_sele->rowCount()>=1){
                                                echo'<a href="#" class="btn btn-default btn-sm"><i class="fa fa-check"></i> Already Enroll </a>';
                                            }
                                            else{
                                                $sc_sel = "SELECT sched_id FROM tbl_schedule WHERE sy_id=:sy_id";
                                                $sc_sele= $db->prepare($sc_sel);
                                                $sc_sele->bindParam(':sy_id',$_SESSION['sy_id']);
                                                $sc_sele->execute();
                                                if($sc_sele->rowCount()>=1){
                                                    $sc_count = $sc_sele->rowCount();
                                                }
                                                else{
                                                    $sc_count = 0;
                                                }

                                                $tl_sel = "SELECT load_id FROM tbl_load WHERE sy_id=:sy_id";
                                                $tl_sele= $db->prepare($tl_sel);
                                                $tl_sele->bindParam(':sy_id',$_SESSION['sy_id']);
                                                $tl_sele->execute();
                                                if($tl_sele->rowCount()>=1){
                                                    $tl_count = $tl_sele->rowCount();
                                                }
                                                else{
                                                    $tl_count = 0;
                                                }
                                                ...
                                                if($sc_count==0){
                                                    echo'<span style="color:red">There was no schedule created this school year</span>';
                                                }
                                                else{
                                                    if($sc_count==$tl_count){
                                                        echo'<a href="index.php?page=app_enroll&stud_id='.$row_sel['stud_id'].'" class="btn btn-success btn-sm"><i class="fa fa-check"></i> Enroll </a>';
                                                    }
                                                    else{
                                                        echo'<span style="color:red">The schedule was not assigned to the teacher.</span>';
                                                    }
                                                }
                                            }
                                        echo'</td>'; 
