                    ...
                    <table class="table table-bordered " style="line-height:10.0px;">
                        ...
                        <?php
                            ...
                            while($su_row=$su_sele->fetch()){
                                ...
                                $f_sel  = "SELECT * FROM tbl_grades WHERE ensub_id=:ensub_id";
                                $f_sele = $db->prepare($f_sel);
                                $f_sele ->bindParam(':ensub_id',$su_row['ensub_id']);
                                $f_sele ->execute();
                                $f_row  = $f_sele->fetch();
                                ...
                                echo'<tr>';
                                    ...
                                    if($f_row['first_quarter']<=79){
                                        echo'<td><label style="font-weight:normal; color:red;">'.$f_row['first_quarter'].'</label></td>';
                                    }
                                    else{
                                        echo'<td><label style="font-weight:normal;">'.$f_row['first_quarter'].'</label></td>';
                                    }
                                    if($f_row['second_quarter']<=79){
                                        echo'<td><label style="font-weight:normal; color:red;">'.$f_row['second_quarter'].'</label></td>';
                                    }
                                    else{
                                        echo'<td><label style="font-weight:normal;">'.$f_row['second_quarter'].'</label></td>';
                                    }
                                    if($f_row['third_quarter']<=79){
                                        echo'<td><label style="font-weight:normal; color:red;">'.$f_row['third_quarter'].'</label></td>';
                                    }
                                    else{
                                        echo'<td><label style="font-weight:normal;">'.$f_row['third_quarter'].'</label></td>';
                                    }
                                    if($f_row['fourth_quarter']<=79){
                                        echo'<td><label style="font-weight:normal; color:red;">'.$f_row['fourth_quarter'].'</label></td>';
                                    }
                                    else{
                                        echo'<td><label style="font-weight:normal;">'.$f_row['fourth_quarter'].'</label></td>';
                                    }
                                echo'</tr>';
                            }
                        ?>
