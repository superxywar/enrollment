<?php
    if(isset($_POST['btn_save'])){
        ...
        $updates->bindParam(':password',$password);
        $updates->bindParam(':stud_id',$_SESSION['stud_id']);
        $updates->execute();
        
        $u_update = "UPDATE tbl_account SET email=:email, password=:password WHERE stud_id=:stud_id";
        $u_updates= $db->prepare($u_update);
        $u_updates->bindParam(':email',$guardian_email);
        $u_updates->bindParam(':password',$password);
        $u_updates->bindParam(':stud_id',$_SESSION['stud_id']);
        $u_updates->execute();
        
		$_SESSION['msf'] = 1;
		header('Location:index.php?page=app_student');
	}
